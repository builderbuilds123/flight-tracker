...
class AddAlertHandlers:
    ...
    async def get_destination(
        ...
        context.user_data[self.user_data_key]["destination_airport"] = (
            text if len(text) == 3 else None
        )
    ...
    def get_conversation_handler(self) -> ConversationHandler:
        ...
        return ConversationHandler(
            entry_points=[
                ...
                MessageHandler(
                    filters.TEXT & filters.Regex(r"^➕ Add Alert$"), self.start
                ),
            ],
            states={
                self.ORIGIN: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, self.get_origin)
                ],
                self.DESTINATION: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.get_destination
                    )
                ],
                self.DEPARTURE_DATE: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.get_departure_date
                    )
                ],
                self.RETURN_DATE: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.get_return_date
                    )
                ],
                self.ONE_WAY_OR_ROUND_TRIP: [CallbackQueryHandler(self.get_trip_type)],
                self.PRICE_THRESHOLD: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.get_price_threshold
                    )
                ],
                self.CONFIRM: [
