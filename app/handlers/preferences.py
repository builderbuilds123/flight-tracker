...
class PreferencesHandlers:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if not preferences:
            await update.message.reply_text(
                "⚠️ Could not load preferences. Try /start first."
            )
    ...
    async def select_currency(
        ...
        context.user_data[self.user_data_key] = context.user_data.get(
            self.user_data_key, {}
        )
    ...
    async def select_timezone(
        ...
        kb = [
            [InlineKeyboardButton(f"{code}", callback_data=f"tz_{code}")]
            for code, _ in timezones
        ]
    ...
    async def cancel(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if update.callback_query:
            ...
            await update.callback_query.edit_message_text(
                "❌ Cancelled.", reply_markup=Keyboards.get_main_menu()
            )
        else:
            await update.message.reply_text(
                "❌ Cancelled.", reply_markup=Keyboards.get_main_menu()
    ...
    def get_conversation_handler(self) -> ConversationHandler:
        ...
        return ConversationHandler(
            entry_points=[
                ...
                MessageHandler(
                    filters.TEXT & filters.Regex(r"^⚙️ Preferences$"), self.start
                ),
            ],
            states={
                self.CURRENCY: [
                    CallbackQueryHandler(
                        self.select_currency, pattern="^pref_currency$"
                    ),
                    CallbackQueryHandler(self.set_currency, pattern="^currency_"),
                ],
                self.TIMEZONE: [
                    CallbackQueryHandler(
                        self.select_timezone, pattern="^pref_timezone$"
                    ),
                    CallbackQueryHandler(self.set_timezone, pattern="^tz_"),
                ],
                self.NOTIFICATION_FREQUENCY: [
                    CallbackQueryHandler(
                        self.select_frequency, pattern="^pref_frequency$"
                    ),
